namespace Flow.Reactive.Streams.Persisted
{
    ...
    public abstract class PersistedStreamDataCollection<T> : PersistedStreamData
    {
        private readonly DataCollection<T> _collection;

        protected PersistedStreamDataCollection() => _collection = new DataCollection<T>();

        protected PersistedStreamDataCollection(IEnumerable<T> initialItems) => _collection = new DataCollection<T>(initialItems);
